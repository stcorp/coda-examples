C After compilation run this program as follows:
C $ ./scil1b | gnuplot
C
      program scil1b_gnuplot
...
C     Since the default output (STDOUT) is used as input for gnuplot we
C     use the error stream (STDERR) for providing feedback to the user.
      integer stderr
      parameter (stderr = 0)
...
C use 'integer*8 num_states' for 64-bit
      ...
      integer num_dsr
      integer dsrNr
C use 'integer*8 mdsrNr' for 64-bit
      integer mdsrNr
      integer cluster
      integer chan_num
      integer pixelNr
...
C CODA allows two ways to read such array data in Fortran:
...
C Method 2 requires some processing in the CODA library and is thus
...
C
      ...
      write(stderr,*) 'Name of the SCIAMACHY Level 1b file:'
      read(*,'(A1024)') filename
     ...
     &    product_type .ne. 'SCI_NL__1P') then
      ...
      write(*,*) 'set xrange [200:2500];'
      write(*,*) 'set yrange [1e-2:1e7];'
      write(*,*) 'set xtics (200,300,400,600,900,1200,1500,2000,2500);'
      write(*,*) 'set logscale y 10;'
      write(*,*) 'set xlabel "wavelength [nm]";'
      write(*,*) 'set ylabel "signal [BU/s]";'
      write(*,*) 'set nokey;'
...
C       Read mds_type, num_clus, and num_dsr
     ...
     &      'num_dsr')
        ...
        write(stderr,*) 'Processing state ', stateNr, ' of ', num_states
        write(stderr,*) '  mds_type .....: ', mds_type, ' (' //
     &    state_name(mds_type)(1:length(state_name(mds_type))) // ')'
        write(stderr,*) '  num_clus .....: ', num_clus
        write(stderr,*) '  num_dsr ......: ', num_dsr
          ...
          do 95, chan_num = 1, 8
            do 100, pixelNr = 1, 1024
              if (badpx(pixelNr, chan_num) .eq. 1) then
                pixel_array(pixelNr, chan_num) = -999.0
              end if
100         continue
95        continue
...
C         Make the gnuplot
          ...
          result = coda_cursor_get_index(mds_cursor(mds_type), mdsrNr)

          write(*,'(1X,A,I4,A,I4,A)') 'set title "' //
     &      filename(1:length(filename)) // ' STATE: ', stateNr, ' (' //
     &      state_name(mds_type)(1:length(state_name(mds_type))) //
     &      ') mdsr: ', mdsrNr + 1, '";'
          write(*,*) 'plot "-" with lines linetype 1, ' //
     &      '"-" with lines linetype 2, "-" with lines linetype 3, ' //
     &      '"-" with lines linetype 4, "-" with lines linetype 5, ' //
     &      '"-" with lines linetype 6, "-" with lines linetype 7, ' //
     &      '"-" with lines linetype 8;'

          do 105, chan_num = 1, 8
            prevOk = .false.
            do 110, pixelNr = 1, 1024
              value = pixel_array(pixelNr, chan_num)
              if (value .lt. 1.0E-2) then
                if (prevOk) then
                  write (*,*) ''
                  prevOk = .false.
                end if
              else
                write(*,*) lambda(pixelNr, chan_num), value
                prevOk = .true.
              end if
110         continue
            write(*,*) 'e'
